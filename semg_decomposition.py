...
def main():
    ...
    # 1. Load data
    sub_emg = hyser.load_1dof(DATA_DIR, subject, session)
    ...
    # 3. FastICA
    if os.path.exists(os.path.join(CACHE_DIR, "emg_sep.npz")):
        data = np.load(os.path.join(CACHE_DIR, "emg_sep.npz"))
        emg_sep = data["emg_sep"]
    else:
        emg_sep, sep_mtx = hyser.fast_ica(emg_white, n_comp, strategy, g_func, max_iter, ica_th, prng, verbose=True)
        np.savez_compressed(os.path.join(CACHE_DIR, "emg_sep.npz"), emg_sep=emg_sep, sep_mtx=sep_mtx)

    # 4. Spike detection
    spike_train = hyser.spike_detection(emg_sep, seed=seed, verbose=True)

    # 5. MU duplicates removal
    valid_index = hyser.replicas_removal(spike_train, emg_sep, FS_EMG)
    spike_train = spike_train[valid_index]
    emg_sep = emg_sep[valid_index]
    print("N. components extracted:", spike_train.shape[0])

    # Compute silhouette
    sil = hyser.silhouette(emg_sep, FS_EMG, seed=seed, verbose=True)
    ...
    hyser.plot_signals(emg_sep_valid[:25], spike_train_valid[:25], n_cols=2, fig_size=(50, 70))
